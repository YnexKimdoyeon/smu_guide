...
`/admin` 경로에서 관리자 페이지 접근 가능

### 관리자 기능
- 전체 통계 대시보드 (유저, 시간표, 채팅 등)
- 통계 상세 보기 (카드 클릭 시 모달)
- 유저 목록 조회 및 검색
- 도토리 순/가입일 순 정렬
- 유저 상세 정보 조회
...
### 인증
- `POST /api/sunmoon/login` - 선문대 포털 로그인
- `GET /api/auth/me` - 내 정보 조회
- `DELETE /api/auth/withdraw` - 회원 탈퇴

### 시간표
- `GET /api/schedules` - 내 시간표 조회
- `POST /api/schedules` - 시간표 추가
- `DELETE /api/schedules/{id}` - 시간표 삭제

### E-러닝 (Canvas)
- `POST /api/canvas/init` - Canvas 세션 초기화
- `GET /api/canvas/courses` - 수강 과목 목록
- `GET /api/canvas/todos` - 할 일 목록
- `GET /api/canvas/courses/{id}/syllabus` - 수업계획서
...
### 도토리
- `GET /api/dotori/info` - 도토리 정보
- `POST /api/dotori/attendance` - 출석 체크
- `POST /api/dotori/shop/purchase` - 아이템 구매
- `GET /api/dotori/ranking` - 학과 랭킹
...
- GitHub Issues: [YnexKimdoyeon/smu_guide](https://github.com/YnexKimdoyeon/smu_guide)
